...
let code_of_attr : attr -> string = function
  | `Bold -> "1"
  | `Faint -> "2"
  | `Underline -> "4"
  | `Fg `Black -> "30"
...
let not_color : attr -> bool = function
  | `Bold | `Faint | `Underline -> true
  | `Fg _ | `Bg _ -> false

let raw_style_string st : string =
  String.concat ""
    [ "\x1b["; String.concat ";" (List.map code_of_attr st); "m" ]
...
let style_string ~param st =
  ...
  | _ -> (
      ...
      match st with [] -> "" | _ -> raw_style_string st)
...
let reset_string ~param st =
  ...
  | _ -> (
      let st = if param.color then st else List.filter not_color st in
      match st with [] -> "" | _ -> raw_style_string [])
...
module Test = struct
  ...
  let is_tty o = try Unix.isatty (Unix.descr_of_out_channel o) with _ -> false

  let guess ?use_ansi ?use_color o =
    let enabled =
      match use_ansi with Some a -> a | None -> rich_term && is_tty o
    in
